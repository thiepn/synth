...
- Compact / Standard / Extended song candidates
- Auto / Beat Family / EVOLVE source selection
- deterministic song seeds and Try Another
- EVOLVE material projected into the existing Beat Family vocabulary
- existing Scene/Section generator reused as the structural engine
- non-destructive whole-song candidate preview
- preview duration normalized to include appended transitions
- section locks that survive full rerolls
- source-lineage-aware lock preservation
- targeted one-section reroll
- candidate energy / route diagnostics
- COMMIT TO ARRANGE promotes the candidate family + blueprint
- committed result becomes the existing editable ARRANGE timeline
...
- Clean / Punchy / Dynamic / Loud Quick Master candidates
- non-destructive level-matched Master A/B
- manual Input / Low / High / Glue / Width / Output / Ceiling controls
- live master signal chain with safety-limiter fallback
- immutable production render snapshots
- Pattern / ARRANGE / Section / Custom range rendering
- deterministic OfflineAudioContext PCM renderer
- one-beat pre-roll and bounded none / auto / fixed tail handling
- sample/hybrid decoding from immutable local bytes
- 16-bit / 24-bit / 32-bit-float WAV encoding
- deterministic TPDF dither for integer PCM only
- exact Pattern loop export
- sequential eight-voice processed stem export in ZIP
- render task lifecycle with stale-result cancellation
- peak / RMS / approximate loudness / clipping analysis
...
- IndexedDB v2 named-version store
- conflict-safe compare-and-save project revisions
- cross-tab newer-revision detection
- Save As escape hatch for conflicted local work
- named snapshots with safe restore-as-new-current-revision semantics
- project duplicate and dependency-safe inactive-project delete
- asset garbage collection across both projects and historical versions
- verified portable .synth.zip project packages
- safe uncompressed ZIP parser with CRC/path/size validation
- SHA-256/content-address verification for imported audio
- backup import as a new project ID by default
- browser storage usage/quota/persistence diagnostics
