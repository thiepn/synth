...
function mutateDna(
  ...
  const drift = (key: keyof KitDNA, delta: number) => {
    next[key] = clamp01(next[key] + delta * a);
  };

  switch (mutation) {
    case "darker":
      drift("brightness", -0.3);
      drift("air", -0.24);
      drift("weight", 0.08);
      drift("depth", 0.08);
      break;
    case "brighter":
      drift("brightness", 0.32);
      drift("air", 0.28);
      drift("tightness", 0.04);
      break;
    case "heavier":
      drift("weight", 0.34);
      drift("depth", 0.24);
      drift("tightness", -0.08);
      break;
    case "cleaner":
      drift("roughness", -0.38);
      drift("variance", -0.16);
      drift("tightness", 0.08);
      break;
    case "dirtier":
      drift("roughness", 0.4);
      drift("variance", 0.18);
      drift("synthetic", 0.08);
      break;
    case "tighter":
      drift("tightness", 0.36);
      drift("depth", -0.14);
      drift("variance", -0.08);
      break;
    case "bigger":
      ...
      drift("depth", 0.34);
      drift("air", 0.12);
      drift("tightness", -0.12);
      break;
    case "stranger":
      drift("synthetic", 0.28);
      drift("variance", 0.42);
      drift("roughness", 0.12);
      break;
  }

  const jitter = source.variance * a * 0.08;
  for (const key of Object.keys(next) as Array<keyof KitDNA>) {
    next[key] = clamp01(next[key] + random.range(-jitter, jitter));
  }
